...
![Version: 1.1.0](https://img.shields.io/badge/Version-1.1.0-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 7.13.2-jdk11](https://img.shields.io/badge/AppVersion-7.13.2--jdk11-informational?style=flat-square)
...
| volumes.additionalVolumeClaimTemplates | list | `[{"enableInConfluence":false,"enableInSynchrony":false,"name":"myadditionalvolumeclaim","resources":{"requests":{"storage":"1Gi"}},"storageClassName":null}]` | Defines additional volumeClaimTemplates that should be applied to Confluence and/or Synchrony pods. Note that this will not create any corresponding volume mounts; those needs to be defined in confluence.additionalVolumeMounts |
| volumes.additionalVolumeClaimTemplates[0] | object | `{"enableInConfluence":false,"enableInSynchrony":false,"name":"myadditionalvolumeclaim","resources":{"requests":{"storage":"1Gi"}},"storageClassName":null}` | Name of the additional volumeClaimTemplate |
| volumes.additionalVolumeClaimTemplates[0].enableInConfluence | bool | `false` | If 'true', then a 'volumeClaimTemplates' will be dynamically created for Confluence based on the 'StorageClassName' supplied below. |
| volumes.additionalVolumeClaimTemplates[0].enableInSynchrony | bool | `false` | If 'true', then a 'volumeClaimTemplates' will be dynamically created for Confluence based on the 'StorageClassName' supplied below. |
| volumes.additionalVolumeClaimTemplates[0].resources | object | `{"requests":{"storage":"1Gi"}}` | Specifies the standard K8s resource requests for the volumeClaimTemplates volume claims. |
| volumes.additionalVolumeClaimTemplates[0].storageClassName | string | `nil` | Specify the name of the 'StorageClass' that should be used for the additional volume claim template. |
