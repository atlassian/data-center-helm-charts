...
| agent | object | `{"additionalLibraries":[],"enabled":true,"image":{"pullPolicy":"IfNotPresent","repository":"atlassian/bamboo-agent-base","tag":""},"ports":{"http":80},"resources":{"container":{"requests":{"cpu":"2","memory":"2G"}},"jvm":{"maxHeap":"2048m","minHeap":"512m"}},"service":{"port":80,"type":"ClusterIP"},"shutdown":{"command":"/shutdown-wait.sh","terminationGracePeriodSeconds":30}}` | Bamboo agent image details |
| agent.additionalLibraries | list | `[]` | Specifies a list of additional Java libraries that should be added to the Synchrony container. Each item in the list should specify the name of the volume that contains the library, as well as the name of the library file within that volume's root directory. Optionally, a subDirectory field can be included to specify which directory in the volume contains the library file. Additional details: https://atlassian.github.io/data-center-helm-charts/examples/external_libraries/EXTERNAL_LIBS/ |
| agent.image.pullPolicy | string | `"IfNotPresent"` | Image pull policy |
| agent.image.repository | string | `"atlassian/bamboo-agent-base"` | The Bamboo Docker image to use https://hub.docker.com/r/atlassian/bamboo-server |
| agent.image.tag | string | `""` | The docker image tag to be used - defaults to the Chart appVersion |
| agent.ports.http | int | `80` | The port on which the Bamboo agent listens |
| agent.resources.container.requests.cpu | string | `"2"` | Initial CPU request by Synchrony pod |
| agent.resources.container.requests.memory | string | `"2G"` | Initial Memory request Synchrony pod |
| agent.resources.jvm.maxHeap | string | `"2048m"` | The minimum amount of heap memory that will be used by the Synchrony JVM |
| agent.resources.jvm.minHeap | string | `"512m"` | The maximum amount of heap memory that will be used by the Synchrony JVM |
| agent.service.port | int | `80` | The port on which the Synchrony K8s Service will listen |
| agent.service.type | string | `"ClusterIP"` | The type of K8s service to use for Synchrony |
