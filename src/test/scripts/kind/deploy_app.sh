...
deploy_postgres() {
  ...
  if [ -f "${DB_INIT_SCRIPT_FILE}" ]; then
    ...
    kubectl cp ${DB_INIT_SCRIPT_FILE} atlassian/${PRIMARY_POD}:/tmp/db-init-script.sql
    kubectl exec ${PRIMARY_POD} -n atlassian -- psql -U ${DC_APP} -d ${DC_APP} -f /tmp/db-init-script.sql
  fi
  ...
  PRIMARY_POD=$(kubectl get pods -n atlassian -l cnpg.io/cluster=${DC_APP}-db,role=primary -o jsonpath='{.items[0].metadata.name}')
  kubectl cp ${TMP_INIT_DIR}/init-db.sql atlassian/${PRIMARY_POD}:/tmp/init-db.sql
  kubectl exec ${PRIMARY_POD} -n atlassian -- psql -U ${DC_APP} -d ${DC_APP} -f /tmp/init-db.sql
