...
At present Bamboo suffers with an issue where the server and status REST endpoints become un-usable when performing an [export/import of large datasets](https://jira.atlassian.com/browse/BAM-18673){.external}. If a data export/import is required this should be done [via a DB dump/restore](https://confluence.atlassian.com/bamboo/moving-your-bamboo-data-to-a-different-database-289277250.html){.external}
